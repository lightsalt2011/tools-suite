...
adb push tz.mbn $workspace
adb push rpm.mbn $workspace
adb push emmc_appsboot.mbn $workspace
adb push boot.img $workspace
...
adb shell "dd if=/dev/block/bootdevice/by-name/tz of=$workspace/tz.mbn.dev count=1 bs=$tz_size"
...
adb remount
sleep 1 
adb push ./comp.sh /system/bin/
#adb push /cygdrive/f/yemz/script/compsys.sh /system/bin/
